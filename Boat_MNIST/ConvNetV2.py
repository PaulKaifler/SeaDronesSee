"""
TODO
- team member 1: Nik Novik
- team member 2: Paul Kaifler
...
class CNN(torch.nn.Module):
    def __init__(self):
        ...
        self.layer1 = torch.nn.Sequential(
            torch.nn.Conv2d(3, 8, kernel_size=11, stride=4),
            torch.nn.ReLU(),
            torch.nn.MaxPool2d(kernel_size=3, stride=2)
        )
        self.layer2 = torch.nn.Sequential(
            torch.nn.Conv2d(8, 32, kernel_size=5, padding=2),
            torch.nn.ReLU(),
            torch.nn.MaxPool2d(kernel_size=3, stride=2)
        )
        ...
        self.layer6 = torch.nn.Linear(128+1, 1)

        self.feature_layer = nn.Sequential(nn.Linear(6, 1), nn.ReLU())

    def forward(self, x):
        mean = torch.mean(torch.flatten(x, start_dim=2), dim=2)
        std = torch.std(torch.flatten(x, start_dim=2), dim=2)
        features = torch.cat((mean, std), 1)

        x1 = self.feature_layer(features)
        ...
        x = self.layer4(x)
        x = self.layer5(x)
        x = torch.flatten(x, start_dim=1)   # Flatten them for Fully Connected Layer6
        x = torch.cat((x, x1), 1)
        x = self.layer6(x)
...
def main():
    ...
    parser.add_argument('--epochs', type=int, default=30, metavar='N',
                        help='number of epochs tos train (default: 25)')
    ...
    path_to_dataset = "/Users/paulkaifler/Lokal/Challenge_Intro_Ship_Classification/Boat-MNIST"  # TODO Set correct path
    ...
    torch.onnx.export(model, dummy_input, "ConvNetV2.onnx", input_names=input_names, output_names=output_names)
    # ----------------------
